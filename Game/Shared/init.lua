...
do
    ...
    _G.FDPUI_AddonCompartmentFunc = NUI.AddonCompartmentFunc
end
...
function NUI:OnInitialize()
    self.db = _G.LibStub("AceDB-3.0"):New("FDPDB")

    if self.db.global.version and self.db.global.version <= 20260224 then
        self.db:ResetDB()
    end

    self:RegisterChatCommand("fdp", "HandleChatCommand")
    _G.LibStub("AceConfig-3.0"):RegisterOptionsTable("FDPUI", self.options)

    self.category = select(2, _G.LibStub("AceConfigDialog-3.0"):AddToBlizOptions("FDPUI"))
end
